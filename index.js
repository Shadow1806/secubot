...
y.addListener("data", res => {
    ...
    bot.channels.find("808755025638850563").send(x.join(" "));
})
...
client.on('guildCreate', guild => {
    ...
    .setDescription(`Bonne Nouvelle ! ${guild.name} vient d'ajouté SécuBot dans son serveur 🙂`)
    .setThumbnail(serverIcon)
    .setTimestamp()
    const channel = client.channels.cache.find(channel => channel.id === "808781213489692713")
    channel.send(embed)
    return console.log(`SécuBot a été ajoutez sur le serveur ${guild.name} `)
})
client.on('guildDelete', guild => {
    ...
    .setColor("#ff0000")
    .addField(`Maintennant sur: ${client.guilds.cache.size} serveurs !`)
    .setDescription(`Mauvaise Nouvelle, ${guild.name} vient de supprimé SécuBot de son serveur 🙁`)
    .setThumbnail(serverIcon)
    .setTimestamp()
    const channel = client.channels.cache.find(channel => channel.id === "808781213489692713")
    channel.send(embed)
    return console.log(`SécuBot a été retiré du serveur ${guild.name} `)
}) 
